...
class DataLoader:
    ...
    def __iter__(self):
        ...
        if self.shuffle:
          # Create shuffled ordering for this epoch
    ...
    def __next__(self):
        ...
        if isinstance(batch_data[0], tuple):
            num_returns = len(batch_data[0])
            batched = []
            for i in range(num_returns):
                data = [batch_data[j][i] for j in range(len(batch_data))]
                batched.append(Tensor(np.stack(data, axis=0)))
            return tuple(batched)
        else:
            return Tensor(np.stack(batch_data, axis=0))
        ### END YOUR SOLUTION
