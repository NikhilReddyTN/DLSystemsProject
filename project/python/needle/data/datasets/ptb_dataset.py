    ...
    def tokenize(self, path, max_lines=None):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for li, line in enumerate(f):
                ...
                for w in words:
                    ids.append(self.dictionary.add_word(w))
        return ids
        ### END YOUR SOLUTION
...
def batchify(data, batch_size, device, dtype):
    ...
    nbatch = arr.size // batch_size
    if nbatch == 0:
        return np.empty((0, batch_size), dtype=np.int64)
    arr = arr[: nbatch * batch_size]
    arr = arr.reshape(batch_size, nbatch).T
    return arr
    ### END YOUR SOLUTION
...
def get_batch(batches, i, bptt, device=None, dtype=None):
    ...
    target_nd = nd.array(target_np, device=device)
    data = Tensor(data_nd, device=device, dtype=dtype)
    target = Tensor(target_nd, device=device, dtype=dtype)
    return data, target
    ### END YOUR SOLUTION