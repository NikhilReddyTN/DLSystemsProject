...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        return NDArray.make(new_shape,new_stride,self.device,self._handle,self._offset)
        ### END YOUR SOLUTION
        ...
        for i in range(1,len(new_shape)+1):
            if i<=len(self.shape):
                if self.shape[-i]==1:
                    ...
                    new_stride[-i]=self.strides[-i]
            else:
                new_stride[-i]=0
        return NDArray.make(new_shape,tuple(new_stride),self.device,self._handle,self._offset)
        ### END YOUR SOLUTION]
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for dim, s in enumerate(slices):
          ...
          new_stride.append(self._strides[dim]*step)
        return NDArray.make(tuple(new_shape),tuple(new_stride),self.device,self._handle,new_offset)
        ### END YOUR SOLUTION
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        new_shape = tuple(
            self.shape[i] + axes[i][0] + axes[i][1]
            for i in range(self.ndim)
        )
        out = NDArray.make(new_shape, device=self.device)
        out.fill(0.0)
        insert_slices = tuple(
            slice(axes[i][0], axes[i][0] + self.shape[i], 1)
            for i in range(self.ndim)
        )
