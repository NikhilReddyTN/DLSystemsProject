...
__device__ inline size_t linear_to_strided_index(size_t gid,
                                                 CudaVec shape,
                                                 CudaVec strides,
                                                 size_t offset) {
  size_t rem = gid;
  ...
  for (int d = static_cast<int>(shape.size) - 1; d >= 0; --d) {
    ...
    int32_t idx = static_cast<int32_t>(rem % dim);
    rem /= dim;
    pos += static_cast<size_t>(idx) * static_cast<size_t>(strides.data[d]);
  }
  return pos;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    size_t in_pos = linear_to_strided_index(gid, shape, strides, offset);
    out[gid] = a[in_pos];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    size_t out_pos = linear_to_strided_index(gid, shape, strides, offset);
    out[out_pos] = a[gid];  // a is compact (size == product(shape))
  }
}
...
void EwiseSetitem(const CudaArray& a, CudaArray* out, std::vector<int32_t> shape,
  ...
  /// BEGIN SOLUTION
  size_t size = 1;
  for (int32_t s : shape) size *= static_cast<size_t>(s);
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out,
  ...
  if (gid < out_size) {
    ...
    for (size_t j = 1; j < reduce_size; ++j)
      if (a[base + j] > max_val) max_val = a[base + j];
    out[gid] = max_val;
  }
}
