...
def epoch_general_ptb(data, model, seq_len=40, loss_fn=nn.SoftmaxLoss(), opt=None,
    ...
    while i < nbatch - 1:
        ...
        total_tokens += tokens
        total_loss += float(loss.numpy()) * tokens
    ...
    return np.float64(avg_acc), np.array([avg_loss], dtype=np.float32)
    ### END YOUR SOLUTION
...
def train_ptb(model, data, seq_len=40, n_epochs=1, optimizer=ndl.optim.SGD,
    ...
    for _ in range(n_epochs):
        last_acc, last_loss = epoch_general_ptb(
            data, model, seq_len=seq_len, loss_fn=loss_fn(), opt=opt,
...
def evaluate_ptb(model, data, seq_len=40, loss_fn=nn.SoftmaxLoss,
    ...
    return epoch_general_ptb(
        data, model, seq_len=seq_len, loss_fn=loss_fn(), opt=None,
